...
export const signIn = async () => {
  try {
    const { isSuccess, output } = await PubSub.pubSub<
      PubSubResponse<GoogleOAuthResponse>
    >("/main/googleoauth/sign-in", ["drive"]);
    if (isSuccess) return output;
    throw new Error(output as any);
  } catch (error) {
    throw error;
  }
};
...
export const getToken = async (refreshToken: string): Promise<string> => {
  try {
    const { isSuccess, output } = await PubSub.pubSub<
      PubSubResponse<GoogleOAuthResponse>
    >("googleoauth/get-token", { scopes: ["drive"], refreshToken });
    if (isSuccess) return output.accessToken;
    throw new Error(output as any);
  } catch (error) {
    throw error;
  }
