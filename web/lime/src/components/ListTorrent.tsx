import { TorrentSearchResult } from "@/pages/TorrentPage";
...
// import WSClient from "@pownthep/pubsub/ws/client";
...
export default function ListTorrent({ torrents }: Props) {
  const [selected, setSelected] = useState("");
  const [currentEp, setEpisode] = useRecoilState(playerState);
  const isVideo = ({ name }: TorrentFile) => name.includes("mkv") || name.includes("mp4");

  const handleClick = async (id: string, name: string) => {
    const { infoHash, files, port } = await pubSub<TorrentAddResponse>("/background/torrent/add", id);
          ...
          {torrents.map(({ title, magnetUri, size, seeds, leechs }, index) => {
            return (
              <Grow in={true} timeout={300 + 50 * index} key={magnetUri}>
                ...
                >
                  <ListItemButton onClick={() => setSelected(magnetUri)} selected={magnetUri === selected}>
                    <ListItemIcon>
                      <img src={title.includes("mkv") ? MKVIcon : MP4Icon} alt="File Icon" height="24" />
                    </ListItemIcon>
                    ...
                    <Tooltip title="Stream Torrent" placement="top">
                      <IconButton aria-label="play" onClick={() => handleClick(magnetUri, title)}>
