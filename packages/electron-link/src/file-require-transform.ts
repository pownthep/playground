...
import astUtil from "ast-util-plus";
...
interface Options {
  baseDirPath: string;
  didFindRequire(moduleName: any, arg1: any): boolean;
  source: string;
  filePath: string;
}

export default class FileRequireTransform {
  private options: Options;
  ast: any;
  lazyRequireFunctionsByVariableName: Map<any, any>;
  ...
  apply() {
    ...
    if (
      ...
      path.extname(this.options.filePath) === ".json"
    ) {
    ...
    recast.types.visit(this.ast, {
      visitCallExpression: function (astPath) {
        if (isStaticRequire(astPath)) {
          const moduleName = (<any>astPath.node.arguments[0]).value;
          const absoluteModulePath = self.resolveModulePath(moduleName);
          if (absoluteModulePath) {
            let relativeModulePath = path
              .relative(self.options.baseDirPath, absoluteModulePath)
              .replace(/\\/g, "/");
    ...
    while (foundReferenceToLazyRequire) {
      ...
      recast.types.visit(this.ast, {
        visitIdentifier: function (astPath) {
          if (
            isTopLevelASTPath(astPath) &&
            self.isReferenceToLazyRequire(astPath)
          ) {
            astPath.replace(
              b.callExpression(
                b.identifier(
                  self.lazyRequireFunctionsByVariableName.get(astPath.node.name)
                ),
                []
              )
            );
  ...
  replaceReferencesToGlobalsWithFunctionCalls() {
    recast.types.visit(this.ast, {
      visitIdentifier: function (astPath) {
        if (
          astUtil.isReference(astPath as any) &&
          !isReferenceToShadowedVariable(astPath) &&
          GLOBALS.has(astPath.node.name)
        ) {
          astPath.replace(
            b.callExpression(b.identifier(`get_${astPath.node.name}`), [])
          );
        }
        this.traverse(astPath);
      },
  ...
  replaceAssignmentOrDeclarationWithLazyFunction(astPath) {
    ...
    while (parentPath != null && parentPath.scope === astPath.scope) {
      ...
      if (parentNode.type === "AssignmentExpression") {
        ...
        if (ifStatementPath) {
          if (
            !this.lazyRequireFunctionsByVariableName.has(parentNode.left.name)
          ) {
            ifStatementPath.insertBefore(
              b.variableDeclaration("let", [
                b.variableDeclarator(
                  b.identifier(lazyRequireFunctionName),
                  null
                ),
              ])
            );
          ...
          replacement = b.assignmentExpression(
            "=",
            b.identifier(lazyRequireFunctionName),
              ...
              b.blockStatement([
                b.returnStatement(
                  b.assignmentExpression(
                    "=",
                    parentNode.left,
                    b.logicalExpression("||", parentNode.left, parentNode.right)
                  )
                ),
              ])
            )
          ...
          replacement = b.functionDeclaration(
            b.identifier(lazyRequireFunctionName),
            [],
            b.blockStatement([
              b.returnStatement(
        ...
        this.lazyRequireFunctionsByVariableName.set(
          parentNode.left.name,
          lazyRequireFunctionName
        );
        return;
        ...
        if (parentNode.id.type === "ObjectPattern") {
          parentPath.replace(
            b.variableDeclarator(parentNode.id, b.objectExpression([]))
          );
                  ...
                  b.returnStatement(
                    b.assignmentExpression(
                      ...
                      b.logicalExpression(
                        "||",
                        property.key,
                        b.memberExpression(parentNode.init, property.key)
                      )
                    )
                  ),
                ])
    ...
    console.warn(
      `##[warning] The reference to the module is replaced with the lazy function, but it was not in an assignment expression or a variable declaration. In some cases the bundle might not work, which you should fix manually: \n ${this.options.filePath} `
    );
  ...
  isReferenceToLazyRequire(astPath) {
    ...
    const lazyRequireFunctionName = this.lazyRequireFunctionsByVariableName.get(
      astPath.node.name
    );
    if (
