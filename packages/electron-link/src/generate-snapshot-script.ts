...
import FileRequireTransform from "./file-require-transform";
import indentString from "indent-string";
import { SourceMapGenerator } from "source-map";

export default async function (cache, options) {
  ...
  while (requiredModulePaths.length > 0) {
    ...
    let relativeFilePath = path
      .relative(options.baseDirPath, filePath)
      .replace(/\\/g, "/");
    ...
    if (!moduleASTs[relativeFilePath]) {
      ...
      const transform = new FileRequireTransform({
        ...
        didFindRequire: (unresolvedPath, resolvedPath) => {
          if (
            options.shouldExcludeModule({
              requiringModulePath: filePath,
              requiredModulePath: resolvedPath,
            })
          ) {
            return true;
          } else {
      ...
      const useCachedTransform = cachedTransform
        ? cachedTransform.requires.every(
            (r) =>
              (transform.resolveModulePath(r.unresolvedPath) ||
                r.unresolvedPath) === r.resolvedPath
          )
        : false;
      ...
      if (useCachedTransform) {
        ...
        try {
          ...
          console.error(
            `Unable to transform source code for module ${filePath}.`
          );
          if (e.index) {
  ...
  let snapshotScript = fs.readFileSync(
    path.join(__dirname, "blueprint.js"),
    "utf8"
  );

  // Replace `require(main)` with a require of the relativized main module path.
  let relativeFilePath = path
    .relative(options.baseDirPath, options.mainPath)
    .replace(/\\/g, "/");
  if (!relativeFilePath.startsWith(".")) {
    relativeFilePath = "./" + relativeFilePath;
  }
  ...
  const definitionsAssignmentStartIndex = snapshotScript.indexOf(
  ...
  const sections = [];
  let sectionStartRow =
    getLineCount(snapshotScript.slice(0, definitionsAssignmentStartIndex)) + 1;
  let definitions = "";
  ...
  for (let i = 0; i < moduleFilePaths.length; i++) {
    ...
    sections.push({
      ...
      endRow: sectionStartRow + lineCount - 2,
    });
    definitions +=
      indentString(`${JSON.stringify(relativePath)}: ${source}`, 4) + ",\n";
  ...
  const sectionsAssignmentStartIndex =
    snapshotScript.indexOf(sectionsAssignment);
  const sectionsAssignmentEndIndex =
    sectionsAssignmentStartIndex + sectionsAssignment.length;
  snapshotScript =
